...
function OrgPage() {
  ...
  const onInvite = async () => {
    ...
    setError(null);
    try {
      await inviteMember({ data: { orgId, email: email.trim(), role } });
      setEmail("");
      reload();
    } catch (e) {
            ...
            {manage ? (
              <section className="mt-8 rounded-md border border-border bg-surface p-4">
                ...
                <p className="mt-1 text-xs text-muted">
                  S'il a déjà un compte Kaiflow, il est ajouté tout de suite. Sinon, une invitation est
                  créée (valable 14 jours).
                </p>
                  ...
                  {detail.invites.map((i) => (
                    <li key={i.id} className="flex items-center justify-between gap-3 px-3 py-3">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span>{i.email}</span>
                        <span className="text-xs text-muted">({i.role})</span>
                      </div>
                      {manage ? (
                        <button
                          type="button"
